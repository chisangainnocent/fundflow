...
export default function IssueLoanForm() {
    ...
    useEffect(() => {
        ...
        const getSingleClient = async () => {
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/get/client/${uid}`);
    ...
    //Set client_id from URL param
    useEffect(() => {
        if (uid) {
            setForm(prev => ({ ...prev, client_id: uid }));
        }
    }, [uid]);
    ...
    const handleSubmit = async () => {
        ...
        // Enhanced validation
        if (!form.client_id) {
            setMessage('❌ Client Field is empty');
            return;
        }
